 ...
 * @author Andrew
 */
public class CSVPanel extends javax.swing.JFrame {
    ...
    private void setItUp() {
        ...
        c.gridx--;
        c.gridy++;
        //Setting the xweight to zero for all else will ensure that the check
        //boxes take up the maximum space
        c.weightx = 0;
        c.fill = GridBagConstraints.VERTICAL;
        pnlUnits.add(spnrUnit1, c);
        c.gridx++;
        c.fill = GridBagConstraints.BOTH;
            ...
            public void actionPerformed(ActionEvent e){
                ...
                if (chkIncludeUnits.isSelected())
                {
                    ...
                    try{
                        ...
                        for (int i = 0; i < NUM_UNIT_ATTRIBUTES; i++)
                        {
                            if (unitAttributes[i].equals("Unit Name") && unitChecks[i].isSelected())
                                writer.write("Unit Name");
                            else if (unitAttributes[i].equals("Attack") && unitChecks[i].isSelected())
                                writer.write("Attack");
                            else if (unitAttributes[i].equals("Defence") && unitChecks[i].isSelected())
                                writer.write("Defence");
                            else if (unitAttributes[i].equals("Movement") && unitChecks[i].isSelected())
                                writer.write("Movement");
                            else if (unitAttributes[i].equals("Hitpoint Bonus") && unitChecks[i].isSelected())
                                writer.write("Hitpoint Bonus");
                            else if (unitAttributes[i].equals("Bombard Strength") && unitChecks[i].isSelected())
                                writer.write("Bombard Strength");
                            else if (unitAttributes[i].equals("Range") && unitChecks[i].isSelected())
                                writer.write("Range");
                            else if (unitAttributes[i].equals("Rate of Fire") && unitChecks[i].isSelected())
                                writer.write("Rate of Fire");
                            else if (unitAttributes[i].equals("Cost") && unitChecks[i].isSelected())
                                writer.write("Cost");
                            else if (unitAttributes[i].equals("Zone of Control") && unitChecks[i].isSelected())
                                writer.write("Zone of Control");
                            else if (unitAttributes[i].equals("Prerequisites") && unitChecks[i].isSelected())
                            {
                        ...
                        for (int n = 0; n < biqFile.unit.size(); n++)
                        {
                            for (int i = 0; i < NUM_UNIT_ATTRIBUTES; i++)
                            {
                                PRTO unit = biqFile.unit.get(n);
                                if (unitAttributes[i].equals("Unit Name") && unitChecks[i].isSelected())
                                ...
                                else if (unitAttributes[i].equals("Prerequisites") && unitChecks[i].isSelected())
                                {
                                    if (unit.getRequiredTech() != -1)
                                        writer.write(biqFile.technology.get(unit.getRequiredTech()).getName());
                                    else
                                        writer.write(NONE);
                                }
                                //TODO: Add in required techs
        ...
        getContentPane().add(btnExport, c);
        c.gridy++;
